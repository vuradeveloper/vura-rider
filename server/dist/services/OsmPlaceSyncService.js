...
exports.syncOsmPlaces = syncOsmPlaces;
exports.startOsmPlaceSync = startOsmPlaceSync;
exports.stopOsmPlaceSync = stopOsmPlaceSync;
exports.getOsmSyncStatus = getOsmSyncStatus;
...
async function ensureTable() {
    ...
    await (0, database_1.execute)(`CREATE TABLE IF NOT EXISTS community_places (
    ...
    await (0, database_1.execute)(`ALTER TABLE community_places ADD COLUMN IF NOT EXISTS source VARCHAR(20) DEFAULT 'community'`).catch(() => { });
    await (0, database_1.execute)(`ALTER TABLE community_places ADD COLUMN IF NOT EXISTS osm_id BIGINT`).catch(() => { });
    await (0, database_1.execute)(`ALTER TABLE community_places ADD COLUMN IF NOT EXISTS changed_at TIMESTAMPTZ`).catch(() => { });
    // Sync milestone storage.
    await (0, database_1.execute)(`CREATE TABLE IF NOT EXISTS osm_sync_state (
...
async function getLastSync() {
    try {
        const rows = await (0, database_1.query)("SELECT value FROM osm_sync_state WHERE key = $1", [milestoneKey()]);
...
async function saveLastSync(d) {
    try {
        await (0, database_1.execute)(`INSERT INTO osm_sync_state (key, value, updated_at)
       VALUES ($1, $2, NOW())
       ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`, [milestoneKey(), d.toISOString()]);
    }
    catch { }
}
function elementPoint(e) {
    if (e?.lat != null && e?.lon != null)
        return { lat: Number(e.lat), lng: Number(e.lon) };
    if (e?.center?.lat != null && e?.center?.lon != null)
        return { lat: Number(e.center.lat), lng: Number(e.center.lon) };
    return null;
}
function buildQueries(since) {
    ...
    for (const kind of KINDS) {
        for (let yi = 0; yi < TILES; yi++) {
            for (let xi = 0; xi < TILES; xi++) {
                ...
                queries.push(`[out:json][timeout:15];${kind}(${s},${w},${n},${e});out center;`);
            }
        }
    }
    return queries;
        ...
        for (const base of OVERPASS_MIRRORS) {
            try {
                ...
                try {
                    ...
                    if (!res.ok)
                        throw new Error(`Overpass ${base} HTTP ${res.status}`);
...
async function syncOsmPlaces() {
    ...
    try {
        ...
        await Promise.allSettled(Array.from({ length: Math.min(MAX_CONCURRENCY, queries.length) }, () => runWorker()));
        const elements = [];
        ...
        for (const el of bounded) {
            ...
            try {
                const r = await (0, database_1.execute)(`INSERT INTO community_places (name, address, lat, lng, source, osm_id, uses_count, changed_at)
           VALUES ($1, $2, $3, $4, 'osm', $5, 1, $6)
           ON CONFLICT (name, lat, lng) DO UPDATE SET
             address = EXCLUDED.address,
             source = 'osm',
             osm_id = EXCLUDED.osm_id,
             changed_at = EXCLUDED.changed_at`, [name, address || null, pt.lat, pt.lng, el.id ? Number(el.id) : null, new Date()]);
                if (r?.rowCount === 1)
                    added++;
            }
            catch {
...
function stopOsmPlaceSync() {
    if (timer) {
        clearInterval(timer);
        timer = null;
    }
}
function getOsmSyncStatus() {
    return {
...
//# sourceMappingURL=OsmPlaceSyncService.js.map