...
exports.PAY_LATER = void 0;
exports.computeCreditLimit = computeCreditLimit;
exports.ensurePayLaterTables = ensurePayLaterTables;
exports.countCompletedRides = countCompletedRides;
exports.enrollPayLater = enrollPayLater;
exports.getPayLaterStatus = getPayLaterStatus;
exports.refreshPayLaterLimit = refreshPayLaterLimit;
exports.payRideWithPayLater = payRideWithPayLater;
exports.runMonthlyCollection = runMonthlyCollection;
exports.simulatePayLaterRide = simulatePayLaterRide;
const database_1 = __importDefault(require("../config/database"));
const database_2 = require("../config/database");
const iVerve = __importStar(require("./iVerveService"));
...
function computeCreditLimit(completedRides) {
    if (completedRides < exports.PAY_LATER.MIN_COMPLETED_RIDES)
        return 0;
    const steps = Math.floor((completedRides - exports.PAY_LATER.MIN_COMPLETED_RIDES) / exports.PAY_LATER.RIDES_PER_STEP);
    return Math.min(exports.PAY_LATER.CAP, exports.PAY_LATER.BASE_LIMIT + steps * exports.PAY_LATER.LIMIT_STEP);
}
async function ensurePayLaterTables() {
  ...
  `);
    ...
    await (0, database_2.execute)(`ALTER TABLE rides ADD COLUMN IF NOT EXISTS payment_status VARCHAR(20)`);
    await (0, database_2.execute)(`ALTER TABLE rides ADD COLUMN IF NOT EXISTS payment_method VARCHAR(20)`);
    await (0, database_2.execute)(`ALTER TABLE rides ADD COLUMN IF NOT EXISTS due_at TIMESTAMPTZ`);
}
...
async function countCompletedRides(dbUserId) {
    const row = await (0, database_2.queryOne)(`SELECT COUNT(*)::text AS total
     FROM rides r
...
async function getAccount(dbUserId) {
    return (0, database_2.queryOne)(`SELECT a.*, u.phone, u.email
     FROM pay_later_accounts a
     LEFT JOIN users u ON u.id = a.user_id
     WHERE a.user_id = $1`, [dbUserId]);
}
function maskAccountNumber(accountNumber) {
    const last4 = accountNumber.slice(-4);
    return `•••• ${last4}`;
}
async function enrollPayLater(dbUserId, input) {
    await ensurePayLaterTables();
    const user = await (0, database_2.queryOne)("SELECT id, phone, email FROM users WHERE id = $1", [dbUserId]);
    if (!user)
    ...
    const priorFrozen = await (0, database_2.queryOne)("SELECT id FROM pay_later_accounts WHERE identity_fingerprint = $1 AND status = 'frozen'", [fingerprint]);
    if (priorFrozen) {
        throw new Error("This identity is not eligible for Pay Later.");
    }
    ...
    if (!mockBypass && completedRides < exports.PAY_LATER.MIN_COMPLETED_RIDES) {
        throw new Error(`Pay Later unlocks after ${exports.PAY_LATER.MIN_COMPLETED_RIDES} completed rides (you have ${completedRides}).`);
    }
    ...
    if (!cardHold.verified)
        throw new Error("Card validation failed.");
    const creditLimit = computeCreditLimit(mockBypass ? Math.max(completedRides, exports.PAY_LATER.MIN_COMPLETED_RIDES) : completedRides);
    const account = await (0, database_2.queryOne)(`INSERT INTO pay_later_accounts
...
async function getPayLaterStatus(dbUserId) {
    await ensurePayLaterTables();
    const account = await getAccount(dbUserId);
    if (!account)
        return { enrolled: false, mode: iVerve.paymentsMode() };
    const openRides = await (0, database_2.query)(`SELECT id, COALESCE(actual_fare, estimated_fare) AS fare,
            created_at, due_at, payment_status
     FROM rides
     WHERE passenger_id = $1 AND payment_method = 'pay_later'
     ORDER BY created_at DESC LIMIT 50`, [dbUserId]);
    const completedRides = await countCompletedRides(dbUserId);
    return {
        enrolled: true,
        mode: iVerve.paymentsMode(),
...
async function refreshPayLaterLimit(dbUserId) {
    await ensurePayLaterTables();
    const account = await getAccount(dbUserId);
    if (!account)
        throw new Error("You are not enrolled in Pay Later yet.");
    const completedRides = await countCompletedRides(dbUserId);
    const newLimit = computeCreditLimit(completedRides);
    const updated = await (0, database_2.queryOne)(`UPDATE pay_later_accounts
     SET credit_limit = GREATEST(credit_limit, $2), updated_at = NOW()
     WHERE user_id = $1
     RETURNING *`, [dbUserId, newLimit]);
    return {
        account: { ...updated, available: Number(updated.credit_limit) - Number(updated.outstanding) },
        completed_rides: completedRides,
    };
}
async function payRideWithPayLater(dbUserId, rideId) {
    await ensurePayLaterTables();
    const account = await getAccount(dbUserId);
    if (!account || account.status !== "active") {
        throw new Error("Pay Later is not active on your account.");
    }
    const ride = await (0, database_2.queryOne)(`SELECT id, passenger_id, status, COALESCE(actual_fare, estimated_fare) AS fare, payment_status
    ...
    if (!isFinite(fare) || fare <= 0)
        throw new Error("This ride has no amount due");
    const client = await (0, database_1.default)().connect();
    try {
        await client.query("BEGIN");
        await client.query(`UPDATE pay_later_accounts
       SET outstanding = GREATEST(outstanding - $1, 0), updated_at = NOW()
       WHERE user_id = $2`, [fare, dbUserId]);
        await client.query(`UPDATE rides SET payment_status = 'paid', due_at = NULL WHERE id = $1`, [rideId]);
...
async function runMonthlyCollection() {
    await ensurePayLaterTables();
    const dueRides = await (0, database_2.query)(`SELECT id, passenger_id, COALESCE(actual_fare, estimated_fare) AS fare
     FROM rides
     WHERE payment_method = 'pay_later'
       AND payment_status = 'pending'
       AND (due_at IS NULL OR due_at <= NOW())`);
    const summary = { attempted: 0, succeeded: 0, failed: 0, overdue: 0, details: [] };
    for (const ride of dueRides) {
        const account = await getAccount(ride.passenger_id);
        if (!account || account.status !== "active")
            continue;
        const countRow = await (0, database_2.queryOne)("SELECT COUNT(*)::int AS n FROM payment_collections WHERE ride_id = $1", [ride.id]);
        ...
        if (result.success) {
         ...
         VALUES ($1, $2, $3, $4, 'success', 'iverve')`, [ride.passenger_id, ride.id, fare, attemptNumber]);
            summary.succeeded++;
            summary.details.push({ rideId: ride.id, attempt: attemptNumber, result: "success", overdue: false });
        }
        else {
            await (0, database_2.execute)(`INSERT INTO payment_collections (rider_id, ride_id, amount, attempt_number, result, provider, reason)
         VALUES ($1, $2, $3, $4, 'failed', 'iverve', $5)`, [ride.passenger_id, ride.id, fare, attemptNumber, result.error || "Collection failed"]);
            const isFinalAttempt = attemptNumber >= exports.PAY_LATER.COLLECTION_ATTEMPTS_BEFORE_OVERDUE;
            summary.failed++;
            summary.details.push({ rideId: ride.id, attempt: attemptNumber, result: "failed", overdue: isFinalAttempt });
            if (isFinalAttempt) {
                await (0, database_2.execute)("UPDATE rides SET payment_status = 'overdue' WHERE id = $1", [ride.id]);
                await (0, database_2.execute)("UPDATE pay_later_accounts SET status = 'frozen', updated_at = NOW() WHERE user_id = $1", [ride.passenger_id]);
                if (account.identity_fingerprint) {
                    await (0, database_2.execute)(`INSERT INTO pay_later_blacklist (identity_fingerprint, reason)
             VALUES ($1, $2) ON CONFLICT (identity_fingerprint) DO NOTHING`, [account.identity_fingerprint, "Pay Later collection failed after multiple attempts"]);
                }
                summary.overdue++;
            }
        }
...
async function simulatePayLaterRide(dbUserId, rideId) {
    await ensurePayLaterTables();
    const account = await getAccount(dbUserId);
    if (!account || account.status !== "active") {
        throw new Error("Pay Later is not active on your account.");
    ...
    if (!ride)
        throw new Error("Ride not found");
    if (ride.status !== "completed")
        throw new Error("Only completed rides can be pay-later");
    const fare = Number(ride.fare);
    if (!isFinite(fare) || fare <= 0)
        throw new Error("This ride has no fare");
    const available = Number(account.credit_limit) - Number(account.outstanding);
    if (fare > available) {
        throw new Error(`This ride (R${fare.toFixed(2)}) exceeds your available Pay Later credit (R${available.toFixed(2)}).`);
    }
    await (0, database_2.execute)(`UPDATE rides
     SET payment_method = 'pay_later', payment_status = 'pending', due_at = $1
     WHERE id = $2`, [lastDayOfMonth(), ride.id]);
    await (0, database_2.execute)(`UPDATE pay_later_accounts SET outstanding = outstanding + $1, updated_at = NOW() WHERE user_id = $2`, [fare, dbUserId]);
    const updated = await getAccount(dbUserId);
    return {
        rideId: ride.id,
...
//# sourceMappingURL=PayLaterService.js.map