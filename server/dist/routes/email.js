...
router.post("/send-verification", async (req, res) => {
    try {
        ...
        res.status(r.ok ? 200 : 502).json(r.ok
            ? { success: true }
...
router.post("/receipt", async (req, res) => {
    try {
        ...
        const ride = await (0, database_1.queryOne)(`SELECT r.id, r.pickup_address, r.destination_address, r.estimated_fare, r.actual_fare, r.status,
              u.email, u.full_name
       FROM rides r JOIN users u ON u.id = r.passenger_id
       WHERE r.id = $1 AND r.passenger_id = $2`, [rideId, userId]).catch(() => null);
        if (!ride?.email) {
