...
async function pruneDevLogs() {
    try {
        await (0, database_1.execute)(`DELETE FROM dev_logs
...
router.post("/", async (req, res) => {
    ...
    try {
        for (const c of cleaned) {
            await (0, database_1.execute)(`INSERT INTO dev_logs (app, device_id, level, tag, message, data)
         VALUES ($1, $2, $3, $4, $5, $6)`, [c.app, c.deviceId, c.level, c.tag, c.message, c.data]);
        }
...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        params.push(limit);
        const rows = await (0, database_1.query)(`SELECT dev_logs.id, dev_logs.app, dev_logs.device_id, dev_logs.level,
