...
router.get("/geocode", auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const [mapboxData, osmData] = await Promise.all([
            MAPBOX_TOKEN
                ? fetch(`${MAPBOX_API}/geocoding/v5/mapbox.places/${encodeURIComponent(q)}.json?access_token=${MAPBOX_TOKEN}` +
                    `&limit=${Math.min(10, limit + 4)}&country=za${proximity}&types=poi,address,locality,district,place`, { headers: { "Accept-Encoding": "gzip" } })
                    .then((r) => (r.ok ? r.json() : null))
                    .catch(() => null)
                : Promise.resolve(null),
            fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=${Math.max(limit, 12)}&q=${encodeURIComponent(q)}` +
        ...
        const scoreResult = (name, addr, rlat, rlng) => {
        ...
        const add = (item) => {
            ...
            if (!existing || existing._score < s)
                merged.set(key, { ...item, _score: s });
        };
        const mbData = mapboxData;
        const mapboxFeatures = (Array.isArray(mbData?.features) ? mbData.features : []).map((f) => ({
        ...
        const osmResults = (Array.isArray(osmData) ? osmData : []).map((item) => ({
