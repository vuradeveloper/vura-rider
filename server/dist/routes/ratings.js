...
router.post("/", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (!rideId || !score) {
            res.status(400).json({ error: "rideId and score are required" });
            return;
        }
        if (score < 1 || score > 5) {
        ...
        const ride = await (0, database_1.queryOne)("SELECT id, passenger_id, driver_id FROM rides WHERE id = $1", [rideId]);
        if (!ride) {
       ...
       ON CONFLICT (ride_id) DO UPDATE SET score = $4, comment = $5`, [rideId, passengerId, driverId, score, comment || null]);
        ...
        if (avg) {
            await (0, database_1.execute)("UPDATE driver_profiles SET rating_avg = $1 WHERE user_id = $2", [avg.avg, driverId]);
        }
        ...
        `);
                ...
                res.status(500).json({ error: "Failed to create ratings table" });
            }
            return;
        }
