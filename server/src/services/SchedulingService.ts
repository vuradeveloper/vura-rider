...
const remindedRideIds = new Set<string>();
...
export async function remindAssignedDrivers(io: SocketIOServer): Promise<number> {
  const due = await query<any>(
    `SELECT r.id, r.scheduled_at, u.firebase_uid AS driver_uid
       FROM rides r
  ...
  let sent = 0;
  for (const ride of due) {
    if (remindedRideIds.has(ride.id)) continue;
    remindedRideIds.add(ride.id);

    if (ride.driver_uid) {
      sendPushToUser(ride.driver_uid, {
 ...
 */
export async function releaseOfflineReservations(io: SocketIOServer): Promise<number> {
  const due = await query<any>(
    `SELECT r.id, r.scheduled_at, u.firebase_uid AS passenger_uid
       FROM rides r
  ...
  for (const ride of due) {
    ...
    if (!upd) continue; // another process already handled it

    if (ride.passenger_uid) {
      sendPushToUser(ride.passenger_uid, {
